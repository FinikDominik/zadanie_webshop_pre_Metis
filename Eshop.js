...
SearchInput.addEventListener('keyup',(e) =>{
  ...
  let ZapisVyhladaneho = () => {
    return (eshop.innerHTML = includesValue.map((x)=>{
        let {id, trieda, Nazov, cena, img } = x;
        return `
        <div id=produkt-id-${id} class="Container_pre_produkt"> 
            <div class="poster-container">
...
KosikSumaryBaseDiv.addEventListener('click', () =>{
 ...
 let Generatekosik = () => {
  ...
  if(kosik.length !== 0){
      return (Div_pre_itemi.innerHTML = kosik.map((x)=>{
       let {id, item } = x;
       let hladaj = ProduktData.find((y) => y.id === id) || [];
       return `
               <div id=produkt-id-${hladaj.id} class="Sumarum_produktu" >
                   <img id="Img_produkt" class="img-fluid" src="${hladaj.img}" alt="">
                   <div>
                       <p>${hladaj.Nazov}</p>
...
let ZápisObsahuKosikaDoSumary = () => {
  ...
  if(kosik.length !== 0){
    return (V_KosikuSumaryList.innerHTML = kosik.map((x)=>{
     let {id, item } = x;
     let hladaj = ProduktData.find((y) => y.id === id) || [];
     return `
     <div id=produkt-id-${hladaj.id} class="ObsahKosikaSumary"  >
                   <img class="img-fluid" src="${hladaj.img}" alt="Produkt_img_${hladaj.Nazov}">
                   <p>  ${hladaj.Nazov} </p>
...
'PSČ: ' + PSČ + '\n' +
'Telefón: ' + Telefón + '\n' +
'email: ' + Kontaktný_email + '\n' +
 'Spôsob_doručenia:  ' + Spôsob_doručenia + '\n' + '\n' +'Objednaný tovar:');
...
kosik.map((x)=>{
  ...
  return console.log('id ' + hladaj.id, hladaj.Nazov + '  cena za ks  ' + hladaj.cena + ' €' + ' pocet zvolených ' + item + ' ks' + 'spolu ' + cemadokopy + ' €')
});
